...
import areamakkah from "../../../public/imgs/saudi.png";
...
const projects: project[] = [
  {
    name: "CareComm",
    description:
      "Automated end-to-end social media workflows, including post scheduling, publishing, and analytics aggregation across platforms like Facebook, Instagram, X and TikTok, boosting marketing team productivity and data accuracy",
    src: carecomm,
    alt: "CareComm social media automation",
    tools: [SiMongodb, SiExpress, SiGraphql],
    link: "https://staging.carecomm.com",
  },
  {
    name: "area makkah",
    description: "Built a smart reporting platform to support the Saudi Army in managing and documenting violations with image, video, and form data storage integrated with Google Sheets.Successfully processed and stored more than 18,000 records within just two months through the platform’s reporting system.",
    src: areamakkah,
    alt: "area makkah",
    tools: [skill1, skill3, skill9, skill10],
    link: "https://area5makkah.com/",
  },
  {
    name: "Buy Ecommerce",
    description:
      "Buy Ecmmerce Application build by ReactJs library and tailwind css easy way to shopping based on api for everything (signup, login , wishlist, carts,payment with Amazing interface for Good user experience",
    src: project1,
    alt: "Ecommerce app",
  ...
  },
  {
    name: "Fahmny quran",
    description:
      "Website That give you Random aya form quran and with tafser , and page of specific surah you want with its tafseer.Built with : React(state Mangment by Redux), Tailwind",
    src: project2,
    alt: "fahmny app tafser of quran",
  ...
  },
  {
    name: "WeatherNews",
    description:
      "Search about weather in any city in the world by arabic language and Dailynews",
    src: project3,
  ...
  },
  {
    name: "game reviews",
    description:
      "Developed a dynamic and interactive website that displays a variety of games using a RESTful API.",
    src: project4,
  ...
  },
  {
    name: "Yummy Meal",
    description:
      "Developed a meals-themed website that showcases dishes from around the world and search how to prepare it, using data retrieved from a API.",
    src: project5,
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      cardsRef.current.forEach((card) => {
        ...
        const onLeave = () => {
          gsap.to([img, title, desc].filter(Boolean), {
            x: 0,
            y: 0,
            duration: 0.6,
            ease: "power3.out",
            ...
            {projects.map((project, ind) => (
              ...
              >
                ...
                >
                  <Image
                    src={project.src}
                    className="w-full object-cover h-[120px] md:h-[180px] group-hover/parent:object-scale-down"
                    alt={project.alt}
                    width={800}
                    height={450}
                    unoptimized={typeof project.src === "string"}
                  />
                  <article className="mb-4">
                    <h3 className="text-xl font-bold">{project.name}</h3>
                    <p className="text-gray-400 line-clamp-2 sm:line-clamp-4 md:group-hover/parent:line-clamp-none">
                      {project.description}
                    </p>
                  </article>
                    ...
                    <figure className="flex gap-2">
                      {project.tools.map((Tool, i) => {
                        ...
                        return typeof Tool === "function" ? (
                          <Tool
                            className="w-5 h-5 inline sm:w-7 sm:h-7"
                            style={{ color: brandColor }}
                            key={i}
                          />
                        ) : (
                          <Image
                            className="w-5 inline sm:w-7"
                            src={Tool}
                            key={i}
                            alt={`tool ${i}`}
                          />
        ...
        >
          ...
          >
            <Image
              ...
              width={800}
              height={256}
              unoptimized={typeof modalProject.src === "string"}
            />
            <div className="p-6 space-y-4">
              <h3 className="text-3xl font-bold text-scolor">
                {modalProject.name}
              </h3>
